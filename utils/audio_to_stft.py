...
def audio_to_stft(audio_wav, target_frames=360):
    ...
    if not mel_real.shape == (target_frames, target_frames):
    ...
    if not mel_imag.shape == (target_frames, target_frames):
    ...
    return torch.tensor([mel_real,mel_imag]).float(), hop_length
